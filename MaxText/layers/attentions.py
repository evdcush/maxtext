...
class AttentionOp(nn.Module):
  ...
  def _get_cache_vars(self, cache_name, batch, heads, kv_head_size, dtype):
    ...
    return key_vars, value_vars, segment_id_var
  ...
  def _get_prefill_cache_vars(self, batch, heads, kv_head_size, dtype):
    ...
    return self._get_cached_vars(cache_name, batch, heads, kv_head_size, dtype)
  ...
  def kv_cache_prefill(self,
    ...
    cached_key_vars, cached_value_vars, cached_segment_id_var = self._get_prefill_cache_vars(batch, heads, kv_head_size, key.dtype)
  ...
  def update_ar_key_value(self,
    ...
    if self.quantize_kv:
      ar_key = quantizations.kv_unquantize(cached_key_var.value, cached_key_scale_var.value, one_token_key.dtype)
      ar_value = quantizations.kv_unquantize(cached_value_var.value, cached_value_scale_var.value, one_token_value.dtype)
  ...
  def kv_cache_autoregressive(self,
    ...
    cached_prefill_key_vars, cached_prefill_value_vars, cached_prefill_segment_id_var = self._get_prefill_cache_vars(self.max_target_length, heads, kv_head_size, key.dtype)
